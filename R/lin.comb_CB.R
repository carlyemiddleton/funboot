...
second_layer <- function(b2, 
                         n.Im=n.Im,grid=grid,model.bs=model.bs,c.bs=c.bs,e.bs=e.bs,pffr.data.bs=pffr.data.bs,
                         formula=formula,re=re,outcome=outcome,lin.comb=lin.comb,calc.preds=calc.preds,PREDS.BS.BS=PREDS.BS.BS){
...
lin.comb_CB <- function(formula,lin.comb=list(), data, spatial.covars = NULL, B1=500, B2=500,
  ...
  for(b1 in 1:B1){
    ...
    second.layer.out <- parLapply(cl , as.list(1:B2), second_layer, 
                                  n.Im=n.Im,grid=grid,model.bs=model.bs,c.bs=c.bs,e.bs=e.bs,pffr.data.bs=pffr.data.bs,
                                  formula=formula,re=re,outcome=outcome,lin.comb=lin.comb,calc.preds=calc.preds,PREDS.BS.BS=PREDS.BS.BS)
    PREDS.BS.BS <- do.call('cbind',second.layer.out)
