...
sim  <- function(i, counts, nPatients, nIm, tauB, delta_prime, cl){
  ...
  for (p in 1:nPatients){
    ...
    for (j in 1:nIm){
      ...
      #remove B cells generated outside of the image window
      if (nrow(B_xy) > 0) {
  ...
  Mult_p = p_res[[2]][1,2] #select the p-value for B cells around A cells

  c(Mult_p = Mult_p)

}

#Simulate for each tauB and delta_prime
tauB_vals <- c(2, 3, 4, 5, 6)
for (tauB_val in tauB_vals) {
  delta_prime_vals <- c(0, tauB_val*1/3, tauB_val*2/3, tauB_val)
  for (delta_prime_val in delta_prime_vals) {
    Sys.time()
    res <- lapply(
    ...
    save(res, file = paste0("res_tauB", tauB_val, "_delta_prime", delta_prime_val, ".RData"))
  }
}
