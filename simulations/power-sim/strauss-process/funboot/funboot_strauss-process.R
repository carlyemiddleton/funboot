...
sim_with_save <- function(iter,
                              ...
                              progress_file,
                              nSim){
  res <- sim(iter, nPatients, nIm, tauB, delta_prime, grid, alpha, B1, B2, nthreads, seed)
...
clusterExport(cl, #export the necessary variables and functions to the parallel workers
              varlist = c(
                          #Variables
                          "nPatients", "nIm", "tauB", "delta_prime",
...
res_list <- parLapplyLB(cl, iters, sim_with_save,
                        nPatients = nPatients, nIm=nIm, tauB = tauB, delta_prime=delta_prime,
