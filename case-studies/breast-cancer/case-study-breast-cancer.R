...
##The functions are written out explicitly here in order to keep results reproducible and self-contained in case of package updates.
...
png('case-study-breast-cancer_bottom.png', width=5000, height=1300, res=300)
bottom_row
dev.off()
...
spatial_cov_data <- preprocess_data(data=breastcancer_data,
                                             from_cell=7, #Endothelial cells
...
T <- max(abs((CB_beta0$target_curve_estimates - 0)/CB_beta0$target_curve_SEs))
mean(CB_beta0$T>T) #p-value for the test of beta0(r)=0
...
T <- max(abs((CB_beta1$target_curve_estimates - 0)/CB_beta1$target_curve_SEs))
mean(CB_beta1$T>T) #p-value for the test of beta1(r)=0
...
T <- max(abs((CB_beta2$target_curve_estimates - 0)/CB_beta2$target_curve_SEs))
mean(CB_beta2$T>T) #p-value for the test of beta2(r)=0
...
T <- max(abs((CB_beta3$target_curve_estimates - 0)/CB_beta3$target_curve_SEs))
mean(CB_beta3$T>T) #p-value for the test of beta3(r)=0
...
T <- max(abs((CB_beta4$target_curve_estimates - 0)/CB_beta4$target_curve_SEs))
mean(CB_beta4$T>T) #p-value for the test of beta4(r)=0
